...
// For input in the boxes in play-with friends version.
boxes.forEach((box) => {
...
// For displaying the vs-computer option.
let choice = document.getElementById('vs-comp');
...
// For playing with friends option
choice = document.getElementById('with-frnds');
choice.addEventListener('click', () => {
...
// For choosing the symbol in vs-computer mode
let player_symbol = "O";
let computer_symbol = "X";
choice = document.getElementById('opt-X');
choice.style.opacity = 0.3;
choice.addEventListener('click', () => {
    player_symbol = "X";
    document.getElementById('opt-O').style.opacity = 0.3;
    document.getElementById('opt-X').style.opacity = 1;
});
choice = document.getElementById('opt-O');
choice.addEventListener('click', () => {
    player_symbol = "O";
    document.getElementById('opt-X').style.opacity = 0.3;
    document.getElementById('opt-O').style.opacity = 1;
});


// Checking if the computer mode is started
let start_comp_mode = document.getElementById('start-vs-comp');
start_comp_mode.addEventListener('click', () => {
    let comp_turn = true;
    if(player_symbol === "O"){
        computer_symbol = "X";
        document.querySelector("#player1").innerText = document.getElementById('player-name').value.toUpperCase();
        document.querySelector("#player2").innerText = "COMPUTER";
        comp_turn = false;
    }
    else{
        computer_symbol = "O";
        document.querySelector("#player2").innerText = document.getElementById('player-name').value.toUpperCase();
        document.querySelector("#player1").innerText = "COMPUTER";
    }
    player2.style.opacity = 0.2;
    document.getElementById('player-selection').style.display = 'none';  
});
